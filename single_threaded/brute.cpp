...
void brute_force_kNN(NNDElement **kgraph, float **vectors, int vec_size, int dim, int k){
    ...
    for (int i=0;i<vec_size;i++){
        ...
        for (int j=0;j<vec_size;j++){
            if (j == i)
                ...
                float distance = euclidean_distance(vectors[i],vectors[j],dim);
        ...
        for (int pos=0;pos<k;pos++){
            ...
            kgraph[i][pos].distance = distances.at(pos).first;
            kgraph[i][pos].label = distances.at(pos).second;
        }
...
int main(){
    ...
    brute_force_kNN(kgraph, arr, n, dim, k);
